...
x,noextra   suppress lookup of extra ref names
a,sha1      also print (abbreviated) SHA1 commit name
"
...
nicerevname() {
	if $withsha1; then
		echo -n "`git rev-parse "$1" | cut -c1-7` "
	fi
...
while (( $revlist_idx < $revlist_n )) && ( (( limit > 0 )) || ! $useshowbranch ); do
	...
	if $withextras; then
		xtra="$( git show-ref -d | grep "^$(git rev-parse $r) " | sed -r 's,^.* (refs/)?,,'; )"
	else
		xtra=""
	fi
	if [ -n "$xtra" ]; then
		subjlist[$revlist_idx]="$s (`echo $xtra`)"
	else
